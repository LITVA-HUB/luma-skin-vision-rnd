...
def _numpy_ridge(design, target, alpha, weights=None):
    x = np.asarray(design, dtype=np.float64)
    y = np.asarray(target, dtype=np.float64)
    w = np.ones(len(x)) if weights is None else np.asarray(weights, dtype=np.float64)
    penalty = np.eye(x.shape[1]) * alpha
    penalty[0, 0] = 0.0
    return np.linalg.solve(x.T @ (w[:, None] * x) + penalty, x.T @ (w[:, None] * y))


def _numpy_objective(design, target, beta, alpha, weights=None):
    x = np.asarray(design, dtype=np.float64)
    y = np.asarray(target, dtype=np.float64)
    b = np.asarray(beta, dtype=np.float64)
    w = np.ones(len(x)) if weights is None else np.asarray(weights, dtype=np.float64)
    residual = y - x @ b
    return float(np.sum(w[:, None] * residual**2) + alpha * np.sum(b[1:] ** 2))
...
def _exhaustive_greedy(design, atoms, target, alpha, count, weights=None):
    chosen = []
    gains = []
    current = design.copy()
    beta = _numpy_ridge(current, target, alpha, weights)
    objective = _numpy_objective(current, target, beta, alpha, weights)
    for _ in range(min(count, atoms.shape[1])):
        candidates = []
        for index in range(atoms.shape[1]):
            if index in chosen:
                continue
            expanded = np.column_stack([current, atoms[:, index]])
            candidate_beta = _numpy_ridge(expanded, target, alpha, weights)
            candidate_objective = _numpy_objective(
                expanded, target, candidate_beta, alpha, weights
            )
            candidates.append((objective - candidate_objective, index, expanded, candidate_objective))
        gain, index, current, next_objective = max(candidates, key=lambda item: (item[0], -item[1]))
        chosen.append(index)
        gains.append(gain)
