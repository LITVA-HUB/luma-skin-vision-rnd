...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--fits',type=int,choices=[18,54],default=54);args=parser.parse_args()
    ...
    gap=0.;arrays=0;comparisons=0;coverage_cases=0;initials={}
    for path in records:
    ...
    result={'status':'PASS','fits':len(records),'exact_prediction_arrays':arrays,'exact_risk_arrays':arrays,
            ...
            'maximum_metric_gap':float(gap),'fitting_only_target_scales_and_same_camera_selection_checked':True,
    ...
    name='audit_mixed.json' if args.fits==18 else 'audit.json'
