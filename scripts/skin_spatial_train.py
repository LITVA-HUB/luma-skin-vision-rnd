...
OUT=ROOT/'docs/benchmarks/skin_spatial_v1'
RUN=ROOT/'experiments/runs/skin_spatial_v1'
PROTOCOL=ROOT/'docs/research/skin_spatial_protocol_v1.md'
SEEDS=[17,29,43]
...
def fit(arm,seed,train,val,protocol_name,evaluation=None):
    ...
    record={'arm':arm,'seed':seed,'protocol':protocol_name,'best_epoch':saved['epoch'],'selection_patient_mean':best,
        ...
        'protocol_sha256':sha(PROTOCOL),'training_script_sha256':sha(Path(__file__)),'model_script_sha256':sha(ROOT/'scripts/skin_spatial_model.py'),
...
def main():
    ...
    files=[PROTOCOL,Path(__file__),ROOT/'scripts/skin_spatial_model.py',ROOT/'tests/test_skin_spatial.py',
