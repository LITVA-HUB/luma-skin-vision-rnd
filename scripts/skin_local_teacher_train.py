...
def main():
    ...
    feature_audit=json.loads((OUT/'feature_audit.json').read_bytes());assert feature_audit['status']=='PASS' and feature_audit['exact_image_token_sets']==1230
    ...
    files += [ROOT/'scripts/skin_local_teacher_model.py',ROOT/'scripts/skin_local_teacher_features.py',ROOT/'tests/test_skin_local_teacher.py',
              OUT/'feature_receipt.json',OUT/'feature_audit.json',ROOT/'scripts/skin_mskcc_data.py',ROOT/'scripts/skin_mskcc_summary_pilot.py',ROOT/'scripts/skin_mskcc_train_pixels.py']
    files += [ROOT/'data/processed'/family/(role+'.npz') for family in ['skin_mskcc_pixels_v1','skin_local_teacher_v1'] for role in ['train','validation']]
    ...
    if args.stage=='mixed':
        ...
        assert len(list(OUT.glob('mixed__*/result.json')))==12
