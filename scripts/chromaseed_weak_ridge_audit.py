...
def audit(run, cache, parent, output):
    ...
    assert tuple(lock["alphas"]) == ALPHAS
    assert tuple(lock["widths"]) == WIDTHS
    checks = {"banks": 0, "stored_readouts": 0, "exact_parent_payloads": 0, "oof_query_rows": 0, "choices": 0, "iterative_trajectories": 0,
    ...
    for role, (fit, held) in roles(data["patient"], data["device"]).items():
        ...
        for fold in (0, 1, 2, None):
            ...
            parent_arrays = nz(parent / directory.relative_to(run) / "models.npz")
            ...
            for name, config in receipt["models"].items():
                ...
                if alpha in (.1, 1., 10.):
                    previous_name = name_for(config["family"], config["seed"], config["width_index"], (.1, 1., 10.).index(alpha), config["steps"])
                    previous = unpack(parent_arrays, previous_name)
                    for field in model:
                        np.testing.assert_array_equal(model[field], previous[field])
                    checks["exact_parent_payloads"] += 1
            ...
            if fold == 0:
                for family in ("local_irls", "midpoint_irls"):
                    ...
                    coefficients, _ = reference_readouts(base, x, y, balanced(data["patient"][rows], data["site"][rows]), family, .0001, 16)
                    for step in (1, 4, 16):
    ...
    assert checks["banks"] == 12 and checks["stored_readouts"] == 8748 and checks["choices"] == 15 and checks["selected_models"] == 45
    assert checks["exact_parent_payloads"] == 2916
    paired = []
    ...
    dependency_paths = ("scripts/chromaseed_perceptual_audit.py", "scripts/chromaseed_perceptual_reference.py", "scripts/chromaseed_kernel_audit.py", "scripts/chromaseed_refine_audit.py", "scripts/skin_local_search_train.py", "src/luma_skin_vision/color.py")
