...
def base_model(model):
    return {k[5:]: v for k, v in model.items() if k.startswith("base_")}


class Predictor:
    """Independent FP64 NumPy implementation of the exported single-model consumer."""
...
def predict_torch(model, x, tokens, device="cuda", all_passes=False):
    b = base_model(model)
    net = Bank(str(model["variant"]), seeds=(17,)).to(device).eval()
...
def sampling(weights, steps):
    w = np.asarray(weights, float)
    if w.ndim != 1 or not np.isfinite(w).all() or np.any(w <= 0):
        raise ValueError("positive finite sampling weights required")
    return np.stack(
        [
            np.random.default_rng(seed + 830003).choice(len(w), size=(steps, BATCH), p=w / w.sum())
            for seed in SEEDS
        ]
    )
