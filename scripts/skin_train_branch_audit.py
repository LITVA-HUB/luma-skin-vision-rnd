...
def main():
    ...
    for path in records:
        ...
        for label in ['best','final']:
            ...
            if label=='final' and r['arm'].endswith('_always'):
                original_arm=r['arm'].split('_')[0]+'3'
                original=ROOT/'experiments/runs/skin_spatial_v1'/f"{r['protocol']}__{original_arm}__s{r['seed']}"/'final.pt'
    ...
    result={'status':'PASS','fits':len(records),'exact_prediction_arrays':arrays,'exact_risk_arrays':arrays,
            ...
            'maximum_metric_gap':float(gap),'identical_always_arm_training_trajectories':identical_trajectories,'fitting_only_target_scales_and_same_camera_selection_checked':True,
