...
def shapes(variant):
    e, h = SPECS[variant]
    return dict(w0=(36, h), b0=(h,), v0=(h, 3), c0=(3,), u=(18, e), e=(e,), g=(3 * e, h))


def rate_factors(steps):
    if not 0 < steps <= HORIZON:
        raise ValueError("steps must be within original8192 horizon")
...
class Predictor:
    """One color36 +64x18 tokens, including patch preparation in every response."""
    ...
    def _run(self, x, tokens):
        p, w = self.prep, self.weights
        xx = ((x - p["x_mean"]) / p["x_std"]).astype(np.float64)
        tt = ((tokens - p["t_mean"]) / p["t_std"]).astype(np.float64)
        local = np.maximum(tt @ w["u"] + w["e"], 0)
        mean = local.mean(-2)
        spread = np.sqrt(((local - mean[..., None, :]) ** 2).mean(-2) + 1e-6)
        pool = np.concatenate((mean, spread, local.max(-2)), axis=-1)
        hidden = np.maximum((xx @ w["w0"] + w["b0"]) + pool @ w["g"], 0)
        result = (hidden @ w["v0"] + w["c0"]) * p["y_std"] + p["y_mean"]
...
class Bank(torch.nn.Module):
    ...
    def parts(self):
        return {k: self.theta[:, sl].reshape(6, *shape) for k, (sl, shape) in self.slices.items()}
    ...
    def load_models(self, models):
        if len(models) != 6:
            raise ValueError("fixed six-slot payload bank required")
        for model in models:
            Predictor(model)
            assert str(model["variant"]) == self.variant
        with torch.no_grad():
            for slot, model in enumerate(models):
                for k, (sl, _) in self.slices.items():
                    self.theta[slot, sl].copy_(
                        torch.as_tensor(model[k].ravel(), device=self.theta.device)
                    )
