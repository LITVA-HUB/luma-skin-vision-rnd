"""Predeclared fitting-only branch follow-up on exposed source roles."""
import os
...
from skin_train_branch_model import TrainingBranchColor as SpatialColor,ARMS

OUT=ROOT/'docs/benchmarks/skin_train_branch_v1'
RUN=ROOT/'experiments/runs/skin_train_branch_v1'
PROTOCOL=ROOT/'docs/research/skin_train_branch_protocol_v1.md'
SEEDS=[17,29,43]


def prediction(model,x,mean,std):
    model.eval();predictions=[];risks=[]
    with torch.no_grad():
        for part in x.split(64):
            p,v,w=model(part)
            risk=((v-p[:,None])*std).square().sum(-1).mul(w).sum(1).clamp_min(0).sqrt()
            predictions.append((p*std+mean).cpu().numpy());risks.append(risk.cpu().numpy())
    return np.concatenate(predictions),np.concatenate(risks)


def selective(error,risk,data):
    ordered=np.argsort(risk,kind='stable');result=[]
    for coverage in [1.,.95,.9,.8,.7,.6]:
        n=int(np.ceil(len(error)*coverage));ix=ordered[:n]
        result.append({'requested_coverage':coverage,'accepted':n,'actual_coverage':n/len(error),
                       **summarize(error[ix],rows(subset(data,ix)))})
    return result
...
def fit(arm,seed,train,val,protocol_name,evaluation=None):
    ...
    record={'arm':arm,'seed':seed,'protocol':protocol_name,'best_epoch':saved['epoch'],'selection_patient_mean':best,
        ...
        'stored_parameters':sum(p.numel() for p in model.parameters()),'active_parameters':model.active_parameters(),'training_active_parameters':model.training_active_parameters(),'branch_at_inference':False,
        ...
        'protocol_sha256':sha(PROTOCOL),'training_script_sha256':sha(Path(__file__)),'model_script_sha256':sha(ROOT/'scripts/skin_train_branch_model.py'),
...
def main():
    ...
    files=[PROTOCOL,Path(__file__),ROOT/'scripts/skin_train_branch_model.py',ROOT/'tests/test_skin_train_branch.py',ROOT/'scripts/skin_spatial_model.py',
           ROOT/'scripts/skin_pair_invariance.py',ROOT/'scripts/skin_pair_train.py',ROOT/'scripts/skin_mskcc_pixels.py',
    ...
    if args.stage=='mixed':
        if lock.exists():raise ValueError('Already started; inspect live handle and results before any resume')
    ...
    for protocol,tr,va,ev in protocols:
        for seed in SEEDS:
            for arm in ARMS:fit(arm,seed,tr,va,protocol,evaluation=ev)
