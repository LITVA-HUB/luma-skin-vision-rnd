...
def token_normalizers(tokens):
    t = np.asarray(tokens, np.float64)
    if t.ndim != 3 or t.shape[1:] != (64, 18) or not len(t) or not np.isfinite(t).all():
        raise ValueError("finite nonempty FIT tokensNx64x18 required")
    return dict(
        t_mean=t.mean((0, 1)).astype(np.float32),
        t_std=np.maximum(t.std((0, 1)), 1e-6).astype(np.float32),
    )


def rate_factors(steps):
    if not 0 < steps <= HORIZON:
        raise ValueError("steps within original32768 horizon required")
