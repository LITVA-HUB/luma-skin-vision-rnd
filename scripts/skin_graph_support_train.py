...
def prediction(model,x,mean,std):
    model.eval();out=[]
    with torch.no_grad():
        for part in x.split(64):out.append((model(part)[0]*std+mean).cpu().numpy())
...
def main():
    ...
    files=[PROTOCOL,Path(__file__),ROOT/'scripts/skin_capture_support.py',ROOT/'scripts/skin_graph_support_model.py',ROOT/'tests/test_skin_graph_support.py',ROOT/'scripts/skin_spatial_model.py',ROOT/'scripts/skin_expert_anchor_train.py']
    ...
    if stage=='prepare':
        ...
        print('Support protocol/code frozen; no image model fitting');return
