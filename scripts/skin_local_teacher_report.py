...
def main():
    ...
    for protocol in ['mixed', 'from_SLR', 'from_ipod']:
        for arm in ARMS:
            rows = [r for r in records if r['protocol'] == protocol and r['arch'] == arm]
            ...
            g['active_head_parameters']=rows[0]['active_head_parameters']
            g['required_teacher_parameters']=rows[0]['required_teacher_parameters']
    ...
    with (OUT/'uncalibrated_risk_coverage.csv').open('w', encoding='utf8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['protocol', 'correspondence', 'seed', 'accepted', 'population', 'coverage', 'mean_delta_e00'])
        for r in records:
            with np.load(RUN/f"{r['protocol']}__{r['arch']}_mse__s{r['seed']}"/'evaluation.npz') as a:
                error = a['error'][np.argsort(a['risk'], kind='stable')]
                for k, v in enumerate(np.cumsum(error)/np.arange(1, len(error)+1), 1):
                    writer.writerow([r['protocol'], r['arch'], r['seed'], k, len(error), k/len(error), float(v)])
    comparisons = []
    for protocol, reference_arm in [('mixed','mixture_mse'),('from_SLR','mixture_mse'),('from_ipod','plain_mse')]:
        for arm in ARMS:
            paired = []; seed_deltas = []
            for seed in [17,29,43]:
                folder = RUN/f'{protocol}__{arm}_mse__s{seed}'
    ...
    lines = ['# Local descriptor and RGB correspondence', '',
    ...
    lines += ['', '| Protocol | Correspondence | Mean at 80% | Active head + required teacher parameters | Peak fit+selection MiB |',
              '|---|---|---:|---:|---:|']
    for g in groups:
        if 'risk80' in g:
            lines.append(f"| {g['protocol']} | {g['arm']} | {g['risk80']:.4f} | {g['active_head_parameters']} + {g['required_teacher_parameters']} | {g['max_train_selection_mib']:.2f} |")
