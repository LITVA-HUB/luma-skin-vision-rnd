...
Preflight amendment before primary training: the original adaptive4x4 pooling
on an already4x4 backbone feature grid calls a nondeterministic CUDA backward.
V5 copies V4 into a separate module, omitting only this identity operation when
the grid is already4x4. CPU output and parameter-gradient equivalence and strict
CUDA full field/second-derivative backward are tested. Other input grid sizes
retain V4 behavior; this deterministic training claim concerns128x128 inputs.
The first integration pilot (ccv5_pilot_s17) failed at this kernel and is retained.
