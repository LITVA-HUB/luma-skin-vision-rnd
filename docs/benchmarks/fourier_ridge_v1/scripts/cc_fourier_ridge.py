...
def fit_filter(hist, target, ridge):
    if ridge <= 0:
...
def run(out):
    ...
    bindings = ["scripts/cc_fourier_ridge.py","scripts/cc_v3_ffcc.py","docs/research/fourier_ridge_protocol.md"]
    write_json(out/"config.json",{"data_sha256":DATA_HASHES,"source_sha256":{p:sha256(root/p) for p in bindings},"train_ids":[rows[selected[i]]["id"] for i in train],"validation_ids":[rows[selected[i]]["id"] for i in val],"protocol":"16 fixed candidates; reused source validation only; no author reproduction"})
    ...
    with threadpool_limits(limits=2):
        for sigma in (1.,2.):
            ...
            for ridge in (.001,.01,.1,1.):
                started = time.perf_counter()
                weight = fit_filter(hist[train],smoothed,ridge)
                score = predict_score(hist[val],weight)
                pmf = torch.from_numpy(score*200).flatten(1).softmax(-1).reshape(-1,64,64)
                elapsed = time.perf_counter()-started
                for mode in ("gray_light","gray_world"):
                    name = f"sigma{sigma}_ridge{ridge}_{mode}"
                    ...
                    metrics = {"id":name,"sigma":sigma,"ridge":ridge,"unwrap":mode,"reproduction":summarize(error),"raw_risk":risk_summary(error,risk),"valid":int(valid.sum()),"n":len(val),"parameters_real":12288,"fit_and_validation_seconds":elapsed}
