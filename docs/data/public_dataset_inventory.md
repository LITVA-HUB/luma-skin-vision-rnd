...
|---|---|---|---|
| [Beyond RGB, original author-linked release](https://zenodo.org/records/16848482) | Paper1680 scenes; inspected archive1686 scene folders, not additional independent scenes. Samsung Galaxy S21 Plus and Oppo Find X5 Pro; HDF5 camera RGB already demosaiced, float32 but audited samples quantized to1/255. Separate chart captures, patch polygons, measured illuminant spectra. | DNG tags including ColorMatrix1/2 and CameraCalibration1/2 verified in loader files; neither CCM nor camera AWB used by initial estimator input path. | Original160.29GB archive; selective47-scene phone payload upper bound3.23GB. CLEARED FOR R&D AND COMMERCIAL MODEL TRAINING, originalCC BY4.0 with attribution. Fits local CPU preparation and4060 thumbnail experiments. Acquisition in progress;44 paired test scenes reserved. [Audit](phone_loader_findings.md). |
| [Zhang et al.2023, original Dryad release](https://doi.org/10.5061/dryad.z8w9ghxg9) |30 real object PNG inputs:15 iPhone SE2 and15 XS Max. Additional synthetic/pH assets and illustrative corrected outputs excluded from input count. No verified instrument-paired absolute-color/illuminant GT for these30 photos. | RAW/CCM/camera metadata completeness not verified. |76.45MB acquired and published checksum verified. CLEARED FOR R&D AND COMMERCIAL MODEL TRAINING under originalCC0, but scientific suitability presently auxiliary repeatability only. No model training or accuracy claim from these files. |
...
V4 update: all three correction/refinement models train only on the same cleared SimpleCube++ source (1,126 fitting and 119 development-validation images). No new dataset or pretrained teacher weights were acquired for V4. The newly acquired INTEL-TAU population remains unused for fitting or numeric error evaluation. The [V4 training-source review](../research/cc_v4_visual_training_sources.md) and [asset-rights ledger](../research/cc_v4_efficient_training_sources.md) distinguish standard Apache-2.0 DINOv2 candidates from restricted Cell/XRay/Apple/intrinsic assets; studying these releases does not mean adopting them. [Measured V4 result and limits](../benchmarks/cc_v4_report.md).
