# `scripts/skin_local_teacher_train.py`

[Архив](../README.md) · [Индекс](../SOURCE_INDEX.md) · [Полный исходник](../../../../scripts/skin_local_teacher_train.py)

> Статическая документация исходника. Наличие теста или функции не означает, что её запуск завершён успешно.

Frozen local teacher correspondence screen; allinference color targets are nativeLab.

SHA-256 исходника: `34a8e794eb8834d96d7603da7da641684819e2fdc91b776de0ec6002f3642f53`. Строк: **123**.
...
from skin_local_teacher_features import load,pack,normalization
...
from skin_local_teacher_model import LocalTeacherColor,ARMS
```
...
[Строка 17](../../../../scripts/skin_local_teacher_train.py#L17)

```python
ARCHES=ARMS
```

[Строка 19](../../../../scripts/skin_local_teacher_train.py#L19)

```python
OUT=ROOT/'docs/benchmarks/skin_local_teacher_v1'
```

[Строка 20](../../../../scripts/skin_local_teacher_train.py#L20)

```python
RUN=ROOT/'experiments/runs/skin_local_teacher_v1'
```

[Строка 21](../../../../scripts/skin_local_teacher_train.py#L21)

```python
PROTOCOL=ROOT/'docs/research/skin_local_teacher_protocol_v1.md'
```

[Строка 22](../../../../scripts/skin_local_teacher_train.py#L22)

```python
SEEDS=[17,29,43]
```

[Строка 23](../../../../scripts/skin_local_teacher_train.py#L23)
...
|---|---|---|---|
| `prediction` | FunctionDef | См. реализацию | [L26](../../../../scripts/skin_local_teacher_train.py#L26) |
| `fit` | FunctionDef | См. реализацию | [L34](../../../../scripts/skin_local_teacher_train.py#L34) |
| `main` | FunctionDef | См. реализацию | [L100](../../../../scripts/skin_local_teacher_train.py#L100) |

## Устройство, вычисление ответа и обучение

Ниже точные определения конструкторов, прямых проходов, формул ёмкости и fit/экспорта. Размерности задаются конструкторами и константами выше; наследуемые операции находятся в перечисленных импортируемых модулях. Повторяющиеся имена относятся к разным классам и различаются строкой исходника.

<details><summary>fit · L34–97</summary>
...
def fit(arch,objective,seed,train,val,protocol_name,evaluation=None):
    ...
    x=torch.from_numpy(pack(train,arch)).cuda();vx=torch.from_numpy(pack(val,arch)).cuda()
    ...
    model=LocalTeacherColor(arch);initial=digest(model.state_dict());model=model.cuda()
    tm,ts=normalization(train)
    model.teacher_mean.copy_(torch.from_numpy(tm).cuda());model.teacher_std.copy_(torch.from_numpy(ts).cuda())
    ...
    for epoch in range(1,81):
        ...
        for offset in range(0,len(a),16):
            ...
            color=(pred-y[ix]).square().mean()
            loss=color+.1*torch.nn.functional.cross_entropy(logits,mode[ix])
    ...
    ev=val if evaluation is None else evaluation;ex=vx if evaluation is None else torch.from_numpy(pack(ev,arch)).cuda()
    ...
    record={'arch':arch,'objective':objective,'seed':seed,'protocol':protocol_name,'best_epoch':saved['epoch'],'selection_patient_mean':best,
        ...
        'stored_parameters':sum(p.numel() for p in model.parameters()),'active_head_parameters':model.active_parameters(),
        'required_teacher_parameters':0 if arch=='plain' else 22056576,'gate_parameters':sum(p.numel() for p in model.gate.parameters()),
        ...
        'protocol_sha256':sha(PROTOCOL),'training_script_sha256':sha(Path(__file__)),'model_script_sha256':sha(ROOT/'scripts/skin_local_teacher_model.py'),
