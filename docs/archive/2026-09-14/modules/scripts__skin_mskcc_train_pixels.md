...
[Архив](../README.md) · [Индекс](../SOURCE_INDEX.md) · [Полный исходник](../../../../scripts/skin_mskcc_train_pixels.py)

> Статическая документация исходника. Наличие теста или функции не означает, что её запуск завершён успешно.

Source-only pixel networks: standard CNN and matched0/3-step patch voting.

SHA-256 исходника: `39eedd75cd1a1c51cfb21d77435cdc7853afd3cdfa904a4298c113bcbf48da9c`. Строк: **125**.
...
|---|---|---|---|
| `digest` | FunctionDef | См. реализацию | [L19](../../../../scripts/skin_mskcc_train_pixels.py#L19) |
| `predict` | FunctionDef | См. реализацию | [L25](../../../../scripts/skin_mskcc_train_pixels.py#L25) |
| `fit` | FunctionDef | См. реализацию | [L36](../../../../scripts/skin_mskcc_train_pixels.py#L36) |
| `main` | FunctionDef | См. реализацию | [L116](../../../../scripts/skin_mskcc_train_pixels.py#L116) |

## Устройство, вычисление ответа и обучение

Ниже точные определения конструкторов, прямых проходов, формул ёмкости и fit/экспорта. Размерности задаются конструкторами и константами выше; наследуемые операции находятся в перечисленных импортируемых модулях. Повторяющиеся имена относятся к разным классам и различаются строкой исходника.

<details><summary>predict · L25–33</summary>
...
<details><summary>fit · L36–113</summary>

```python
def fit(arch,seed,train,val,epochs):
    run=ROOT/f'experiments/runs/skin_mskcc_pixels_v1/{arch}_seed{seed}'
    out=ROOT/f'docs/benchmarks/skin_mskcc_pixels_v1/{arch}_seed{seed}'
    ...
    else:model=PatchVotes(target_std,0 if arch=='votes_mean' else 3)
    ...
    def inputs(data):
        ...
        return torch.from_numpy(data['tokens']).float().cuda()
    ...
    record={'arch':arch,'seed':seed,'epochs':epochs,'best_epoch':best_epoch,
            ...
            'full':summarize(error,rows),'strata':{},'protocol_sha256':sha(PROTOCOL),
            'training_script_sha256':sha(Path(__file__)),'vote_script_sha256':sha(ROOT/'scripts/skin_mskcc_vote.py'),
    ...
    if arch!='cnn':
        ...
        record['mean_vote_dispersion_delta_e76']=float(risk.mean())
